 ...
 * @param {string} options The options for.
 * @param {boolean} usingFlatConfig `true` if the config is flat, `false` if not.
 * @returns {Promise<LintResult[]>} The results of the linting.
 */
function spawnLintWorker(files, options, usingFlatConfig) {
    return new Promise((resolve, reject) => {
        const lintWorker = new Worker(path.resolve(__dirname, "cli-lint-worker.js"), {
            workerData: {
                files,
                options,
                usingFlatConfig
        ...
        lintWorker.on("message", resolve);
        lintWorker.on("error", reject);
    });
}
...
const cli = {
    ...
    async execute(args, text, allowFlatConfig) {
        ...
        const translatedOptions = await translateOptions(options, usingFlatConfig ? "flat" : "eslintrc");

        const engine = new ActiveESLint(translatedOptions);
        let results;
        ...
        if (useStdin) {
            ...
            const filesPerWorker = Math.ceil(files.length / cpusLength);

            debug("Spawning %d workers to lint %d files", cpusLength, files.length);

            results = (await Promise.all(Array.from({ length: cpusLength }).map((_, i) => {
                const workerFiles = files.slice(i * filesPerWorker, (i + 1) * filesPerWorker);

                return spawnLintWorker(
                    workerFiles,
                    translatedOptions,
                    usingFlatConfig
                );
